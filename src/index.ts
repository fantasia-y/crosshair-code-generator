...
const DICTIONARY_LENGTH = BigInteger(DICTIONARY.length);
...
const checkCode = (code: string): boolean => code && !!code.match(SHARECODE_PATTERN);

const decode = (code: string): Crosshair => {
  ...
  try {
    ...
    let big = BigInteger(0);
    chars.forEach((_, i) => {
      big = big.multiply(DICTIONARY_LENGTH).plus(BigInteger(DICTIONARY.indexOf(chars[i])));
    });
...
const encode = (crosshair: Crosshair): string => {
  let bytes = [
    1,
    (crosshair.gap * 10),
  ...
  let total = BigInteger(hex, 16);

  let c = '';
  let rem = BigInteger(0);
